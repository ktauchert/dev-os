...
This app lives under the **DevOS** monorepo. Product, scope, AWS split, and UI theme are defined at the repo root—not here.

## Authority (read first)

| Topic | Location |
| --- | --- |
| Product, MVP, milestones | [../AGENTS.md](../AGENTS.md), [../docs/mvp.md](../docs/mvp.md), [../docs/plan.md](../docs/plan.md) |
| Folder layout, features | [../docs/frontend-architecture.md](../docs/frontend-architecture.md) |
| shadcn themes (paper light, dark accents) | [../docs/ui-theme.md](../docs/ui-theme.md), skill [../.cursor/skills/devos-ui-theme](../.cursor/skills/devos-ui-theme/) |

**Stack here:** Vite SPA · TanStack Router (file routes) · TanStack Query · TanStack Table · shadcn/ui · ESLint.

**Not in this app:** TanStack Start, server functions, tRPC, Clerk/Better Auth (use **AWS Cognito** later), Neon/Prisma. API = **API Gateway + Lambda** ([../docs/aws-architecture.md](../docs/aws-architecture.md)). Deploy = `vite build` → S3 + CloudFront.

## TanStack Intent (below)

The block below is TanStack’s **Intent skill index**. When you change routing, loaders, Query, or Table in this package, run the matching `npx @tanstack/intent@latest load …` command for that task—then implement following DevOS docs above.

**Use often:** `@tanstack/router-core#router-core/*` (especially auth-and-guards, data-loading), Query patterns from your app setup.

**Ignore for DevOS unless explicitly migrating:** `react-start`, `start-core/*` (SSR, server routes, server functions, Start deployment adapters).
